...
const USERS='public."MyPrep users"';
const SESSIONS='public."MyPrep sessions"';
...
async function cleanup(){await pool.query(`DELETE FROM ${SESSIONS} WHERE expires_at<=NOW()`)}
async function makeSession(userId){const token=crypto.randomBytes(32).toString("hex"),expires=new Date(Date.now()+SESSION_MS);await pool.query(`INSERT INTO ${SESSIONS}(token_hash,user_id,expires_at) VALUES($1,$2,$3)`,[hashToken(token),userId,expires]);return{token,expires}}
...
async function auth(req,res,next){try{await cleanup();const token=req.cookies.myprep_session;if(!token)return res.status(401).json({error:"Login required."});const q=await pool.query(`SELECT u.id,u.username,u.progress,s.expires_at FROM ${SESSIONS} s JOIN ${USERS} u ON u.id=s.user_id WHERE s.token_hash=$1 AND s.expires_at>NOW()`,[hashToken(token)]);if(!q.rows.length){res.clearCookie("myprep_session");return res.status(401).json({error:"Session expired. Please log in again."})}req.user=q.rows[0];next()}catch(e){console.error("AUTH_DB_ERROR",e);res.status(500).json({error:"Account service unavailable."})}}
